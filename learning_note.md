...
- **分布式限流防护**：针对核心评分接口，基于 **Redis ZSet + Lua 脚本** 实现了**分布式滑动窗口限流器**。
- **技术深度**：利用 Lua 脚本保证限流逻辑在分布式环境下的**原子性**；相比传统固定窗口算法，有效消除了临界点突发流量（Peak Traffic）的安全隐患。
- **解耦设计**：采用 **AOP + 自定义注解** 模式，实现限流逻辑与业务逻辑的零侵入解耦，支持按 UserID/IP 多维度灵活配置。
