...
从“0 错误率”到“发现静默失败”再到“429 状态码优化”

1. **现象描述**：在对推荐接口进行 100 并发压测时，JMeter 聚合报告显示 Error 率为 0%，但实际业务预期应为 97% 拦截。
2. **定位过程**：
   - 通过 JMeter **“查看结果树”** 抓取 Response Body。
   - 发现业务状态码为 400，但由于 HTTP 响应头为 200，导致测试工具误判。
3. **解决方案**：
   - **方案 A（测试端）**：在 JMeter 中添加 **“响应断言”**，检查 JSON 里的 `"code": 200`，强制纠正判别逻辑。
   - **方案 B（服务端）**：优化 `GlobalExceptionHandler`，在触发限流异常时，使用 `HttpServletResponse` 显式设置 HTTP 状态码为 `429 (Too Many Requests)`。
4. **最终成果**：成功将 Error Rate 恢复至真实数值（97%），量化验证了限流器的可靠性。
...
**性能调优与故障排查**：在对核心接口进行压力测试时，针对**“压测结果与业务预期不符”**的静默失效问题，通过分析 **HTTP 协议状态码与业务 JSON 载荷**的差异，定位并解决了因全局异常处理不当导致的测试误报。

**高并发防护验证**：利用 **JMeter** 验证了 **Lua+Redis 限流器** 的拦截精度。在 100 QPS 冲击下，系统精准识别并拦截了 95% 以上的超限流量，保障了数据库与计算资源的平稳运行。
...
> #### 你是如何处理 CI/CD 中的性能测试拦截的？

- 答：“在集成限流功能后，由于业务预期会拦截超额流量并返回 429 状态码，这会导致自动化构建工具（如 JMeter-Maven-Plugin）因错误率超标而判定构建失败。我通过**调整错误率阈值（errorRateThresholdInPercent）**并设置 **ignoreResultFailures**，将‘业务预期报错’与‘系统故障报错’解耦，确保了流水线的稳定性。”