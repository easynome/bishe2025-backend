...
- **架构设计**：基于 **Spring AOP + 自定义注解** 实现了非侵入式的操作日志监控，覆盖核心业务接口。
- **性能优化**：针对日志入库带来的性能损耗，设计了 **异步解耦方案**（TaskExecutor + @Async），实现主线程“零等待”返回，确保系统高吞吐。
- **系统稳定性**：处理了 AOP 上下文丢失、大数据量序列化异常等生产环境常见坑位。通过对请求参数、返回结果进行 **智能过滤与长度自适应截断**，保障了日志系统的极端健壮性。

![image-20260119184642962](C:\Users\85346\AppData\Roaming\Typora\typora-user-images\image-20260119184642962.png)

#### **【性能与容灾】异步线程池的监控与保护**

- **监控设计**：通过 `AsyncConfig` 中的 `monitorThreadPool` 观察任务流动，通过 `getCompletedTaskCount()` 验证任务是否真实执行。
- **拒绝策略**：显式配置 `CallerRunsPolicy`，在极端高并发（队列>200）时让主线程介入，产生“背压”信号，防止系统雪崩。
- **资源释放**：配置 `setWaitForTasksToCompleteOnShutdown`，优雅关闭线程池，解决进程停止时内存中剩余日志丢失的问题。
